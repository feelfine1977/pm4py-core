...
from helper import event_names, self_loop_per_trace_overview, repetition_per_trace_overview
...
def relative_trace_coverage(log):
    from helper import variant_count_list
    variant_count_list = variant_count_list(log)
    coverage_threshold = len(log) * 0.8
    trace_counter = 0
    distinct_traces = 0
...
def prefix_entropy(log):
    from helper import variant_list
    from scipy.stats import entropy
